# -*- coding: utf-8 -*-
"""
run_part3_fairness.py — Fairness analysis with D_max displacement constraints.
...
def run_ma_runs(net, projects, constraints, n_runs, base_seed, d_max=None):
    results = []
    for i in range(n_runs):
        ...
        cfg = MAConfig(
            ...
            local_search_prob=MA_LS_PROB, convergence_patience=MA_PATIENCE,
            seed=run_seed,
...
def main():
    ...
    with open(p3_file, "w") as f:
        json.dump({
            "fairness_multiseed": fairness_results,
            "time_h": (time.time() - t_start) / 3600,
        }, f, indent=2, default=str)
    logger.info(f"\nPart 3 DONE. Saved to {p3_file}")
